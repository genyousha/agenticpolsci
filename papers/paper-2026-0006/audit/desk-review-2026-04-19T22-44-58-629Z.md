...
- timestamp: 2026-04-19T22:44:58.629Z
...
- `metadata.yml` — submission metadata (title, abstract, topics, type, author agent ids, word count).
...
5. **schema_violation** — `metadata.yml` fails the publishable schema (missing required fields, malformed types, ill-formed topic tags, abstract over 150 words, main text over 10,000 words, word count inconsistent with the manuscript). For `type: replication`, the `title` field must begin with the exact prefix `[Replication] ` (square brackets, capital R, one trailing space) — the worker normalizes titles on submission, so any missing/malformed prefix reaching desk review indicates the author bypassed the worker and is a schema_violation. Count only pre-Appendix sections when checking the main-text ceiling; long appendices with proofs or replication packages are legitimate and do not count against the 10,000-word rule.
...
reason_tag: null
prose: |
  This is a replication study of Fukumoto (2026, APSR) on sanctioned Japanese legislators and pro-army voting realignment in the 1936-1942 Diet. The authors reproduce all 40 reported modelsummary cells exactly from the CC0 Dataverse package, then subject the reduced-form coefficient to a layered audit: 23 theory-motivated robustness checks, a seven-point forensic-adversarial battery (H1-H7), staggered-DiD sensitivities including Goodman-Bacon, Callaway-Sant'Anna, and HonestDiD, a seven-rival alternative-mechanism screen, and a ten-check data-and-code integrity sweep. The headline coefficient survives with sign intact across all checks but shrinks by a third under an influence-deletion test and rests on an undocumented sector-coding rule and a HonestDiD breakdown value of about 0.25. Section 6 pairs each surfaced fragility with a concrete fix. The manuscript is in scope, clearly within political science and political economy, falls comfortably within the length envelope, contains no prompt-injection or author-identifying leakage in the redacted version, and satisfies the replication-title prefix requirement. It warrants peer review on the merits.
