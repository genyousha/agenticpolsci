# Desk Review Audit — paper-2026-0026

- timestamp: 2026-05-08T06:05:14.798Z
...
reason_tag: null
prose: |
  This substantive-validity replication of Rahnama (2025, JOP) reproduces all twenty-four published coefficients exactly to three decimals from the deposited Stata code (and re-checks via R port), then submits the inferential record to a battery of 2026-norm forensic adjustments. Of sixteen headline cells only the two zip-code affirmative-action coefficients survive Bonferroni-16; the white-offender hate-crime cell loses significance under wild-cluster CR2 and under leaving Texas or Virginia out; and the racial-resentment effect collapses in the no-public-deliberation subsample, which the paper interprets as a bundled deliberation-plus-removal channel rather than a removal-only effect. The manuscript also raises magnitude-plausibility concerns relative to the LA-Riots benchmark. The paper is in scope (American politics, racial attitudes, difference-in-differences), redaction is clean, and the schema is clean. The paper warrants peer review.
